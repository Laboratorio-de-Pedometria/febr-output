# # Load and publish datasets to the SoilData Dataverse instance
...
index[dados_licenca == "CC BY 4.0", dados_licenca_uri := "http://creativecommons.org/licenses/by/4.0"]
index[dados_licenca == "CC BY-NC 4.0", dados_licenca_uri := "http://creativecommons.org/licenses/by-nc/4.0/"]
index[dados_licenca == "CC BY-NC-SA 4.0", dados_licenca_uri := "http://creativecommons.org/licenses/by-nc-sa/4.0/"]
...
# for (i in seq_len(nrow(index))) {
  ...
  meta$datasetVersion$metadataBlocks$citation$fields[[3]]$value[[1]]$otherIdValue$value <- index[i, dados_id]
  ...
  for (k in seq_along(authors)) {
    meta$datasetVersion$metadataBlocks$citation$fields[[4]]$value[[k]]$authorName$value <- authors[k]
    if (k == 1) {
      if (is.na(index[i, organizacao_nome])) {
        index[i, organizacao_nome := ""]
      }
      index[i, organizacao_nome := gsub(" (EMBRAPA)$", "", index[i, organizacao_nome])]
      index[i, organizacao_nome := gsub("^Embrapa$", "Empresa Brasileira de Pesquisa Agropecuária", index[i, organizacao_nome], perl = TRUE)]
      index[i, organizacao_nome := gsub("EMBRAPA SOLOS", "Embrapa Solos", index[i, organizacao_nome])]
      index[i, organizacao_nome := gsub(" (IBGE)$", "", index[i, organizacao_nome])]
      meta$datasetVersion$metadataBlocks$citation$fields[[4]]$value[[k]]$authorAffiliation$value <- index[i, organizacao_nome]
    } else {
  ...
  meta$datasetVersion$metadataBlocks$citation$fields[[6]]$value[[1]]$dsDescriptionValue$value <- index[i, dados_descricao]
  # subject
  ...
  for (k in seq_along(keywords)) {
    meta$datasetVersion$metadataBlocks$citation$fields[[8]]$value[[k]]$keywordValue$value <- keywords[k]
  }
  # topicClassification
  if (is.na(index[i, area_conhecimento])) {
    index[i, area_conhecimento := ""]
  }
  meta$datasetVersion$metadataBlocks$citation$fields[[9]]$value[[1]]$topicClassValue$value <- index[i, area_conhecimento]
  # publication
  ...
  for (k in seq_along(publication)) {
    meta$datasetVersion$metadataBlocks$citation$fields[[10]]$value[[k]]$publicationCitation$value <- publication[k]
  }
  # language
  ...
  meta$datasetVersion$metadataBlocks$citation$fields[[12]]$value[[1]]$grantNumberAgency$value <- index[i, fonte_financeira]
  ...
  meta$datasetVersion$metadataBlocks$geospatial$fields[[1]]$value[[1]]$otherGeographicCoverage$value <- index[i, extensao_espacial]
